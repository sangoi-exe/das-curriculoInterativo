                    ...
                    <table class="detection-table">
                        <tbody>
                            <tr v-for="(step, index) in filteredDetectionSteps" :key="index">
                                <td>{{ step.message }}</td> <!-- Acessar usando .message -->
                                <td style="textAlign: left">{{ '.'.repeat(step.dots) }}</td> <!-- Acessar usando .dots -->
                                <td style="textAlign: left">{{ step.none ? 'None' : '' }}</td> <!-- Acessar usando .none -->
                            </tr>
                        </tbody>
                    </table>
...
const memoryTest = () => {
    ...
    const interval = setInterval(() => {
        if (waiting) {
            if (waitCounter < 50) { // 50 * 10ms = 500ms
                waitCounter++;
            } else {
                memory.value = 0;
                iterations++;
