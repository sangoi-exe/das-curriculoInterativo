            ...
            <div class="msdos-prompt">
                ...
                <div v-if="!waitingForEnter" class="inpute">
                    <p class="cdoispontosNew" ref="inputScroll">C:\DevArqSangoi&gt;
                        <input v-model="inputValue" @keydown.enter="handleCommand(inputValue)" ref="inputFocus"
                            v-intersection-observer="focaNoInput" />
                    </p>
                </div>
                <div v-if="waitingForEnter" class="pressEnter" @keydown.enter="handleEnter" tabindex="0" ref="focusPressEnter"
...
const translations = {
    ...
    "pt-br": {
        ...
        "unknown_command": "Comando não reconhecido. Digite 'help' para ver a lista de comandos disponíveis.",
...
async function handleHelp() {
    try {
        ...
        console.error("Erro ao carregar arquivo:", error);
        state.commandHistory.push({ command: "help", response: "Erro ao carregar o conteúdo de ajuda. Tente novamente mais tarde." });
    }
}
...
async function handleList() {
    try {
        ...
        console.error("Erro ao carregar arquivo:", error);
        state.commandHistory.push({ command: "list", response: "Erro ao carregar o conteúdo de ajuda. Tente novamente mais tarde." });
    }
}
...
function handleLanguage(langCommand) {
    ...
    state.commandHistory.push({ command: langCommand, response: translations[langCode].language_switched });
